function [U, V] = runMe(videoName, numberOfFrames)
...
in.movieType = videoName; % assumes a file 'LipVid.avi' in current folder. 
...
for frame = 1:numberOfFrames
    [im, u, v, macroDat] = getSavedFlow(frame, 'SavedOutput');
    subplot(1,2,1);imagesc(im); colormap gray;
    axis image;title('video frame');
    subplot(1,2,2);imagesc(sqrt(u.^2+v.^2));colormap gray;
    axis image; title('Motion magnitude')
    
% subplot(1,2,2);imagesc(v);colormap gray;
% axis image;title('V');
% subplot(1,2,1);imagesc(u);colormap gray;
% axis image;title('U');
end
